...
public interface ApplicantMatchRepository extends JpaRepository<ApplicantMatch, Long> {
    ...
    @Query(value = "select DISTINCT(a.participant_id), a.applicant_id, a.match_id, a.join_date " +
            "from applicant_matches a " +
            "inner join applicant_rankings ar on a.participant_id = ar.applicant_match_id " +
            "where a.match_id = ?1 and ar.position_id = ?2" , nativeQuery = true)
